...
from .forms import CreateRideForm, UpdateRideForm, CreateRideReportForm
...
class RidesDetailView(LoginRequiredMixin, DetailView):  # SingleObjectMixin,FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            ...
            elif action == 'create_trip_report':
                return redirect('ride-report-create', pk=pk)
...
class RidesReportCreateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    model = Ride
    template_name = 'rides/report_create.html'
    # fields = ['route', 'ride_date', 'start_time', 'additional_details', 'leader']
    form_class = CreateRideReportForm
    ...
    def form_valid(self, form):
        ...
        if form.has_changed():
            ...
            if self.request.POST['action'] == 'create':
                success_message = 'Ride report created.'
            elif self.request.POST['action'] == 'create_and_email':
                for changed in form.changed_data:
                    print(changed)
                participants = self.get_object().participants.all()
                for p in participants:
                    print("Ride report created. Send email to inform " + p.email)
                success_message = 'Ride report created'
